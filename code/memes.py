...
count_abuse=0
count_hate=0
with open('/home/kshitij/Downloads/dataset_final/dataset.csv', 'r') as myfile:
    reader = csv.reader(myfile , delimiter=',')
    for row in reader:
        if(len(row)<2):  ##########
            continue
...
train = pd.read_csv('/home/kshitij/Downloads/dataset_final/dataset.csv')
train_image = []
yi=[]
with open('/home/kshitij/Downloads/dataset_final/dataset.csv', 'r') as f:
    mycsv = csv.reader(f)
    t = list(mycsv)
    
    for i in tqdm(range(train.shape[0]+1)):
    #for i in tqdm(range(100)):
        img = image.load_img('/home/kshitij/Downloads/dataset_final/images/'+t[i][0], target_size=(50,50,3) )
